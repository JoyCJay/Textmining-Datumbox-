...
class DatumBox():
	...
	def keyword_extract(self, text):
		...
		response = self._send_request(full_url, {'text' : text, 'n' : 3})
	...
	def _send_request(self, full_url, params_dict):
		...
		data = bytes(urllib.parse.urlencode(params_dict), encoding='utf8')
		f = urllib.request.urlopen(urllib.request.Request(url=full_url, data=data))  
